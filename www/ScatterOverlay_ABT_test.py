...
def overlayFig(fig, ax4, df_temp,  exc_common, theta):
    # plot overlay figures with orange = non-DE, green = DE genes
    '''
    df_temp = 2-column DF as gene expression in 2 dif conditions, first column as x-axis
    exc_common0 = list of non-overlapping genes, without p-value filter. This is the result from deRun function below. p-value calculated in R
    exc_common = list of non-overlapping genes, with p-value filter. This is the DE genes
    theta = scatter dot size
    '''
    
    df_exc = df_temp.loc[exc_common, :]    # DE genes = # non-overlapping genes with p-value filter (p-value from R code)
    df_inc = df_temp.loc[~df_temp["Names"].isin(exc_common), :]
...
def scatterFig(fig, ax, df_temp, theta, color='darkred'):
    # plot actual figures
...
def runFig(df_temp, df_temp2, df_temp3, exc_common, theta):
    colorlist = ['seagreen', 'rebeccapurple', 'darkred', 'orange']
    fig = plt.figure(figsize=(6,6) )
    
    fig.subplots_adjust(hspace=0.3, wspace=0.3)    
    ax = fig.add_subplot(2,2,1); ax = scatterFig(fig, ax, df_temp2, theta, color=colorlist[1] )
...
def deRun(df, log_base=10, theta = 0.0125, cutoff_val = 0):
    df = pd.DataFrame(df) 
    columns = [0, 1, 2, 3]
    # colorlist = ['seagreen', 'rebeccapurple', 'darkred', 'orange']
    ...
    plt.show()
    '''
    return df, df_temp, df_temp2, df_temp3, exc1_names, exc2_names, exc_common

'''
theta = 0.0125
file_name = "ecoli_tpm_cutoff_5.csv"
data = pd.read_csv(file_name)
df = pd.DataFrame(data)
df = df.set_index('Unnamed: 0')
df.index.name = None

df, df_temp, df_temp2, df_temp3, exc1_names, exc2_names, exc_common = deRun(df, theta = theta)
runFig(df_temp, df_temp2, df_temp3, exc_common, theta)
