...
def overlay(df_temp, df_2_3, theta):
    ...
    x_val_inc = []; x_val_exc1 = [];
    y_val_inc = []; y_val_exc1 = [];
    exc1_names = [];
...
def overlayFig(df_temp, exc1_names, theta, new_file_img=None):
    # plot overlay figures with orange = non-DE, green = DE genes
    # df_temp = two-column df, column1 = rep2_cond1, column2 = rep2_cond2
    df_exc = df_temp.loc[exc1_names, :]; # print(df_exc.shape); print(df_exc.head())
    df_inc = df_temp.loc[~df_temp["Names"].isin(exc1_names), : ]; # print(df_inc.shape); print(df_inc.head())

    fig4, ax4 = plt.subplots(1, 1, num='Fig 4', figsize=(3,3))
#     ax4.set_title('Overlapping scatter - Log base = ' + str(log_base))
...
#     x_min, x_max, y_min, y_max = ax4.axis()
    ...
    size_pt4 = (2*rpix4/fig4.dpi*72)**2
    pts_inc.set_sizes(size_pt4)
    pts_exc.set_sizes(size_pt4)
    
    if (new_file_img != None):
        fig4.savefig(new_file_img,dpi=1200, bbox_inches='tight') #dpi=1200,

    plt.show()
    return()

def scatterFig(df_temp, theta, color='darkred' ,new_file_img=None):
    # plot actual figures
    fig, ax = plt.subplots(1, 1, num='Fig 1', figsize=(3,3))
#     ax.set_title(df_temp.columns[0] + '/' + df_temp.columns[1] + ' Scatter')
    ...
    if (new_file_img != None):
        fig.savefig(new_file_img , dpi=1200, bbox_inches='tight') #dpi=1200,
...
# parser.add_argument('-colours', type=str, default='seagreen,rebeccapurple,darkred,orange', help='Scatter plot colour')
...
colourlist = ['seagreen', 'rebeccapurple', 'darkred', 'orange']
columns = [int(args.columns.split(',')[0])-1, int(args.columns.split(',')[1])-1, int(args.columns.split(',')[2])-1, int(args.columns.split(',')[3])-1]
...
df_diff_genes = pd.concat([df_diff_genes,pd.DataFrame(exc2_names), pd.DataFrame(exc_common)], ignore_index=True, axis=1)
...
# save image file
new_file_img0 = file_name[:-4] + "_log_" + str(log_base) + "_size_" + str(round(theta,4)) + "_scatter0.png"
new_file_img1 = file_name[:-4] + "_log_" + str(log_base) + "_size_" + str(round(theta,4)) + "_scatter1.png"
new_file_img2 = file_name[:-4] + "_log_" + str(log_base) + "_size_" + str(round(theta,4)) + "_scatter2.png"
new_file_img3 = file_name[:-4] + "_log_" + str(log_base) + "_size_" + str(round(theta,4)) + "_overlay.png"

scatterFig(df_temp=df_temp, theta=theta, color=colourlist[0],  new_file_img=new_file_img0)
scatterFig(df_temp=df_temp2, theta=theta, color=colourlist[1], new_file_img=new_file_img1)
scatterFig(df_temp=df_temp3, theta=theta, color=colourlist[2], new_file_img=new_file_img2)
overlayFig(df_temp=df_temp, exc1_names=exc_common, theta=theta,new_file_img=new_file_img3)    
